...
UserCommand(integer iNum, string sStr, key kID, integer fromMenu) {
    ...
    if (sCmd == "menu") {
        ...
        else llMessageLinked(LINK_DIALOG,NOTIFY,"0"+"The license card has been removed from this cuff. Please find the recent revision [https://raw.githubusercontent.com/OpenCollarTeam/OpenCollar/master/LICENSE here].",kID);
        ...
        if (iNum == CMD_OWNER || kID == g_kWearer ) {   //primary owners and wearer can lock and unlock. no one else
            ...
            llMessageLinked(LINK_DIALOG,NOTIFY,"1"+"%WEARERNAME%'s cuff has been locked.",kID);
            llMessageLinked(LINK_ROOT,LM_CUFF_CMD,"Lock=on","");//Line added to control slave cuffs locks
        }
        ...
        if (iNum == CMD_OWNER)  {
            ...
            SetLockElementAlpha(); //EB
            llMessageLinked(LINK_DIALOG,NOTIFY,"1"+"%WEARERNAME%'s cuff has been unlocked.",kID);
            llMessageLinked(LINK_ROOT,LM_CUFF_CMD,"Lock=off","");//line added to control slave cuffs locks
        }
        else llMessageLinked(LINK_DIALOG,NOTIFY,"0"+"%NOACCESS%",kID);
        if (fromMenu) MainMenu(kID, iNum);
    } else if (sCmd == "fix") {
        ...
        if (kID == g_kWearer) {
            ...
            llMessageLinked(LINK_DIALOG,NOTIFY,"0"+"Only the wearer can update the cuff.",kID);
            if (fromMenu) HelpMenu(kID, iNum);
        }
...
init (){
    ...
    handle = llListen(menuChan,"",g_kWearer,"");
}
...
default {
    ...
    touch_start(integer num)//so we can touch the cuff
    {
        ...
        if (id == g_kWearer && resize == TRUE)// if we are wearer then allow to resize
            llDialog(g_kWearer,"Select if you want to Resize this item or the main Cuff Menu ",["Resizer","Cuff Menu", "Disable Resize"],menuChan);
        else
            llMessageLinked(LINK_AUTH, CMD_ZERO, "menu", id);//else just give cuff menu
    }
    ...
    listen(integer channel, string name, key id, string message) {
        if (llGetOwnerKey(id) == g_kWearer && channel == g_iUpdateChan) {   //collar and updater have to have the same Owner else do nothing!
        ...
        if (id == g_kWearer && channel == menuChan)
        {
            if (message == "Cuff Menu")
                llMessageLinked(LINK_AUTH, CMD_ZERO, "menu", id);
            else if (message == "Disable Resize")
            {
                llListenRemove(menuChan);
                resize = FALSE;
            }
            else if (message == "Resizer")
                makeMenu();
            else
            {
                if (message == "RESTORE")
                    cur_scale = 1.0;
                else if (message == "MIN SIZE")
                    cur_scale = min_scale;
                else if (message == "MAX SIZE")
                    cur_scale = max_scale;
                else
                    cur_scale += (float)message;
